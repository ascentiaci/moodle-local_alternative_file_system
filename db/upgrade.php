...
function xmldb_local_alternative_file_system_upgrade($oldversion) {
    ...
    if ($oldversion < 2024110501) {
        ...
        if (!$dbman->table_exists("local_alternative_file_system_file")) {
            $table = new xmldb_table("local_alternative_file_system_file");
        ...
        if ($dbman->table_exists("local_alternative_file_system_file")) {
            $sql = "INSERT
                       INTO {local_alternative_file_system_file} (contenthash, storage, timemodifield)
                     SELECT contenthash, storage, timemodifield FROM {local_alternative_file_system_file}";
            $DB->execute($sql);
